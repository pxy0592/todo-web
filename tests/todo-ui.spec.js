...
test.describe("todo semantic UI", () => {
  ...
  test("shows the empty state when no tasks are rendered", async ({ page }) => {
    ...
    await expect(
      page.getByRole("status", { name: "任务列表状态" }),
    ).toContainText("暂无任务");
    await expect(page.getByRole("listitem")).toHaveCount(0);
  });

  test("renders each task row with its checkbox and delete control", async ({
    page,
  }) => {
    await page.goto("/");
    ...
    await expect(
      rows.nth(0).getByRole("checkbox", { name: "购买牛奶" }),
    ).toBeVisible();
    await expect(
      rows.nth(0).getByRole("button", { name: "删除购买牛奶" }),
    ).toBeVisible();
    await expect(
      rows.nth(1).getByRole("checkbox", { name: "整理书桌" }),
    ).toBeVisible();
    await expect(
      rows.nth(1).getByRole("button", { name: "删除整理书桌" }),
    ).toBeVisible();
    await expect(
      page.getByRole("status", { name: "任务列表状态" }),
    ).toBeHidden();
  ...
  test("exposes completed styling hooks and checked state", async ({
    page,
  }) => {
    await page.goto("/");
    await renderFixture(page, [
  ...
  test("renders HTML-like task text literally without creating an image", async ({
    page,
  }) => {
    await page.goto("/");
    ...
    await expect(
      row.getByRole("button", { name: `删除${taskText}` }),
    ).toBeVisible();
  });
});

test("keeps long task content and row controls usable at a narrow viewport", async ({
  page,
}) => {
  ...
  const taskText =
    "这是一个足够长的任务标题，用来验证窄屏下内容会换行而不是挤出操作区域";
...
test.describe("todo interactions and persistence", () => {
  test("adds a task with a click, clears the input, and restores it after reload", async ({
    page,
  }) => {
    await page.goto("/");
  ...
  test("deletes only the targeted task and keeps it deleted after reload", async ({
    page,
  }) => {
    await page.goto("/");
    const input = page.getByRole("textbox", { name: "任务" });
  ...
  test("shows a non-blocking status when saving fails without losing the task", async ({
    page,
  }) => {
    ...
    await expect(page.getByRole("status", { name: "操作状态" })).toContainText(
...
test.describe("todo storage initialization recovery", () => {
  test("shows a load warning for unsupported saved data and still allows adding a task", async ({
    page,
  }) => {
    await page.addInitScript(() => {
    ...
    await expect(page.getByRole("status", { name: "操作状态" })).toContainText(
      "Saved tasks have an unsupported format.",
    );
    await expect(page.getByRole("textbox", { name: "任务" })).toBeVisible();
  ...
  test("uses an in-memory fallback when localStorage is unavailable", async ({
    page,
  }) => {
    await page.addInitScript(() => {
