...
test("allocates a unique ID when the injected factory collides", () => {
  const existing = [{ id: "existing", text: "Existing", completed: false }];
  const ids = ["existing", "new-task"];

  const result = addTask(existing, "New task", () => ids.shift());
