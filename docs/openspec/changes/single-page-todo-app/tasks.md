...
- [x] 2.2 Implement versioned `localStorage` serialization, validation, loading, and saving; verify tests cover initial empty state, valid restoration, invalid JSON/shape recovery, and persistence after each mutation.
...
- [x] 3.3 Connect form submission, Enter-key submission, checkbox changes, and delete actions through event delegation; verify browser integration tests cover add, blank submission, complete/uncomplete, and delete behavior.
- [x] 3.4 Load persisted tasks during page initialization and save state after every mutation; verify an end-to-end refresh restores text and completion state and does not restore deleted tasks.
